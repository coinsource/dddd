...

{
    ...
    sph_echo512_close(&ctx_echo, static_cast<void*>(&hash[10]),pbegin);




    //add now for change first =fisrt xor five four ,got four speed;
    uint256 Vb11 =hash[10].trim256();
   // printf("fffffffffffffffffhash %s",Vb11.GetHex().c_str());
    uint256  Vb12 = (Vb11 & (uint256(0xff)<<216) )<<32;//xor five byte
    Vb11 =(Vb11 ^ Vb12) &  ( uint256("0xf3ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")); //four x11 speed
  //  printf("dddffhash %s",Vb11.GetHex().c_str());
    return Vb11;
}
