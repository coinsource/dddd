...
unsigned int nTargetSpacing = 1 * 60; // 30 seconds
unsigned int nRetarget = 10; //
unsigned int nStakeMinAge = 15 * 60; // 15 minute
...
const string strMessageMagic = "Rockcoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if(pindexBest->nHeight < 24000)
    {
        nSubsidy = 125 * COIN;
    }
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
  ...
  //  if (GetBlockTime() > FutureDrift((int64_t)vtx[0].nTime))
     //   return DoS(50, error("CheckBlock() : coinbase timestamp is too early nTimeBlock=%"PRId64" nTimeTx=%u", GetBlockTime(), (int64_t)vtx[0].nTime));
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Rockcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "mycoin";
        CTransaction txNew;
        txNew.nTime = 1404617231;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 10 << CBigNum(42) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        block.nVersion = 1;
        block.nTime    =1404617231;// 1402929449;//1404617231
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2171623;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("04be0295800b1ff09ad8ffd13c400eeb9f188dc6aa2b6a2131fc3f61a4c88f17"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x11, 0x22, 0x33, 0x44 };
